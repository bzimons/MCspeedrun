...
import logging #@
logging.basicConfig(filename='LOG_brc.log', encoding='utf-8', level=logging.WARNING) 
logging.warning('Logging prog 229') #@ 

dbg = True # ver linha 410 onde altera o label para segundos 
import datetime
...
class _BarChartRace:
    ...
    def prepare_data(self, df):
        ...
        if self.fixed_order:                      #@ aqui a ordem é fixa não vai rolar FIXA FIXA 
            n = df_values.shape[1] + 1
            m = df_values.shape[0]
            rank_row = np.arange(1, n) # rank_row = np.arange(1, n)
            if (self.sort == 'desc' and self.orientation == 'h') or \
                (self.sort == 'asc' and self.orientation == 'v'):
                rank_row = rank_row[::-1]
 
            #logging.warning('175:', rank_row[::]) #@ 
            #rank_row = rank_row[::-1] #@ forcei a ordem 
            #bar_location = np.sort(bar_location)[::-1]
...
################################################################################          
    def plot_bars(self, i):
        ...
        if self.orientation == 'h':
            #bar_location = np.sort(bar_location)[::-1]   #@ ordena as barras 
            #logging.warning(bar_location)
            self.ax.barh(bar_location, bar_length, tick_label=cols+">",  #@ tick_label é do eixo Y
                         color=colors, **self.bar_kwargs) #@ forcar_cor
            if not self.fixed_max:
                self.ax.set_xlim(self.ax.get_xlim()[0], bar_length.max() * 1.1)
        else:
            self.ax.bar(bar_location, bar_length, tick_label='*', 
                        color='y', **self.bar_kwargs) #@
            if not self.fixed_max:
        ...
        if self.label_bars:
            ...
            #logging.warning("380:", list(zipped))
            for x1, y1 in zipped:
...
def prepare_wide_data(df, orientation='h', sort='desc', n_bars=None, interpolate_period=False,   #@ desc BIA
    ...
    logging.warning("DF_VALUES", df_values) #@ rank rankk 

    df_ranks = df_values
    for i in range(len(df_values)):
        ser1 = df_values.iloc[i]
        ser2 = ser1.rank(ascending = 0)
        df_ranks.iloc[i] = ser2

    logging.warning("DF_VALUES ############# modificado", df_values) #@ 935

    return df_values, df_ranks
        
#    if compute_ranks:
#        logging.warning("DF_RANKS", df_ranks) #@
#        return df_values, df_ranks
#    return df_values
