...
import datetime # inserida essas 4 linhas e linha 382 com o label em time 
def get_sec(time_str):
    h, m, s = time_str.split(':')
    return int(h) * 3600 + int(m) * 60 + int(s)

import logging as lg 
lg.basicConfig(filename='LOG_brc.log', encoding='utf-8', level=lg.WARNING) 
lg.warning('Log prog 320 casa') #@ 
...
class _BarChartRace:
    ...
    def prepare_data(self, df):                                         
        ...
        compute_ranks = self.fixed_order is False     # obviamente calcule on ranks se não tem ordem fixa
        lg.warning(f"prepare data:{compute_ranks}")
        ...
        lg.warning(f"prepare data - df_values:{df_values}")
        lg.warning(f"prepare data - df_ranks:{df_ranks}")

        return df_values, df_ranks


    def get_col_filt(self):                                           #@ é aqui que ele filtra 
        ...
        if self.n_bars < self.df_ranks.shape[1]:
            ...
            if orient_sort in [('h', 'asc'), ('v', 'desc')]:
                ...
                #lg.warning('col_filt:', col_filt, type(col_filt))            
            else:
                # 1 is low
    ...
    def plot_bars(self, i):                             #@ filtra as barras O CORRETO AQUI SERIA CRIAR UM LOW_FILT
        bar_location = self.df_ranks.iloc[i].values     # top_filt - numpy.ndarray
        
        lg.warning(f"ORI:{bar_location}") # 
        #bar_location = self.n_bars + 1 - bar_location                      # FEITA A INVERSAO MAS COM MENOS BARRAS ELE EXPULSA A DE CIMA 
        top_filt = (bar_location > 0) & (bar_location < self.n_bars + 1)    # SE COLOCAR 3 ELE MANTEM OS MAIORES 
                                                                            # NO ORI ELE BOTA ZERO SE LIMITAR AS BARRAS
        lg.warning(bar_location) # [5. 4. 1. 2. 3.]
        lg.warning(top_filt)     # [False  True  True  True  True]
        ...
        if self.orientation == 'h':
            self.ax.barh(bar_location, bar_length, tick_label="=>"+cols,   # bar-location-5 comprime as barras
                         color=colors, **self.bar_kwargs)  # +100
        ...
        if self.label_bars:  # label nas barras
            if self.orientation == 'h':
            ...
            for x1, y1 in zipped:
                ...
                if self.orientation == 'h':
                    xtext += .01
                    tempo = str(datetime.timedelta(seconds=x1*100))
                    text = f'<<{tempo}>>'  #@ aqui formata text = f'<{x1:,.0f} segundos>'
                    rotation = 0
                    ha = 'left'
                    va = 'center'
                else:
                ...
                text = self.ax.text(xtext, ytext, text, ha=ha, rotation=rotation, fontsize=self.bar_label_size, va=va)
...
def prepare_wide_data(df, orientation='h', sort='desc', n_bars=None, interpolate_period=False, 
    ...
    lg.warning("DF_VALUES", df_values)
    lg.warning("DF_RANKS", df_ranks)    
