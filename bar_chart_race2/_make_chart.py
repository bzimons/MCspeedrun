...
class _BarChartRace:
    ...
    def calculate_new_figsize(self, real_fig):
        ...
        if self.orientation == 'h':
            ...
            ax.set_yticklabels(self.df_values.columns, rotation=45)
    ...
    def plot_bars(self, i):
        ...
        if self.orientation == 'h':
            ...
            self.ax.set_yticks(bar_location) #add
            self.ax.set_yticklabels(cols,horizontalalignment ='left',color='w',position=(0.05,0)) #add
        ...
        if self.label_bars:
            ...
            for x1, y1 in zipped:
                ...
                if self.orientation == 'h':
                    ...
                    tempo = ('{:02}m {:02}s {:03} ms'.format( s % 3600 // 60, s % 60, ms)) # format of time 
                    text = f'{tempo}'  
                    rotation = 0
                    ha = 'left'
                    va = 'center'
                ...
                text = self.ax.text(xtext, ytext, text, ha=ha, rotation=rotation, fontsize=self.bar_label_size, va=va,color='w')
...
def prepare_wide_data(df, orientation='h', sort='desc', n_bars=None, interpolate_period=False, 
    ...
    if compute_ranks:
        df_ranks = df_values.rank(axis=1, method='first', ascending=True).clip(upper=n_bars + 1) # Changed here to ascending=True
